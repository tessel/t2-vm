...
exports.create = function(opts) {
  ...
  .then(function (bridge) {
    if(opts.image)
    {
      var imgPath = path.resolve(opts.image);
      shell.info('Copying image: ' + imgPath);
      
      function failed(error) {
        throw error;
      }
      
      var rd = fs.createReadStream(imgPath);
      rd.on('error', failed);

      var wr = fs.createWriteStream(etc.PATH_VM_VDI);
      wr.on('error', failed);
      wr.on('close', function(ex) {
        shell.success(' copied ...');
        next();
      });
      rd.pipe(wr);
    }
